...
- Detalhes completos, decisões e medições:
  ...
  [`docs/handoffs/2026-09-17-entradas-card-e-atalho-nfl.md`](handoffs/2026-09-17-entradas-card-e-atalho-nfl.md).

### Objetivo e o que entrou

Tela Entradas funcional, com resultados e encerramento, nas duas marcas:

- Item Entradas da navbar clicável de novo; quem entra na tela fica logado.
- Card sem preenchimento, com o brilho do Pulse; 16px entre os chips e o primeiro card; divisor
  vertical no cabeçalho.
- **Apostas feitas no protótipo entram em Próximas** (`useMyEntries` + `createEntryFromReceipt`);
  Vencedoras e Anteriores têm exemplos fixos. Só a primeira de Próximas nasce aberta.
- **Encerrar aposta:** Cancelar/Confirmar → carregamento de 2s → "Aposta encerrada por" e tag
  ENCERRADA → o card se recolhe → sai para a aba nova **Encerradas**.
- **Jogo encerrado** (nós Figma 1993:6893 e 1993:6923): "Final", placar, check verde ou X vermelho
  ao lado da odd, PA verde na certa, barra da estatística do jogador; tags GANHOU!, NÃO GANHOU
  (valor riscado) e CANCELADO (seleções apagadas, "Reembolso").
- **Card recolhido** (nó 4125:63570): cabeçalho e um círculo por escolha, com anel e selo de
  acerto/erro; accordion animado entre aberto e recolhido.
- Navbar com o novo ícone de Apostas e as versões ativas de Apostas, Entradas e Cassino.
- Textos da Draftea do card traduzidos, com auditoria do Jev (fora do app).
- Correções nas linhas compartilhadas com a tela de sucesso: as cores de acerto/erro e o fundo da
  bolinha do trilho do parlay dependiam da raiz `.bet-success`.

### Validações

- No navegador, viewport 375×812, Pitaco e Draftea: fluxos de aposta real → Próximas, encerramento
  completo, abas, recolhido e aberto — medições no histórico detalhado.
- `tsc -b`, `eslint` dos arquivos tocados, `npm run check:brands`, auditoria local de textos e
...
- Tarefa separada sugerida: textos trocados de idioma no betslip da Pitaco ("Para ganar",
  "Desliza para apostar") e na tela de sucesso da Draftea ("APOSTA CRIADA!").
...
- [2026-09-18 — Entradas: funcionalidade, resultados e encerramento](handoffs/2026-09-18-entradas-funcionalidade.md):
  apostas do protótipo em Próximas, encerrar aposta com a aba Encerradas, placar e acerto/erro de
  jogo encerrado, card recolhido com círculos, accordion, ícones ativos da navbar e a auditoria de
  textos da Draftea com o Jev. Traz as medições de tempo e as decisões tomadas com a pessoa
  responsável.
- [2026-09-17 — Tela Entradas: card da aposta e atalho `/nfl`](handoffs/2026-09-17-entradas-card-e-atalho-nfl.md):
  o card do nó Figma 1993:6822 sobre as linhas do recibo, a extração das peças de seleção
  compartilhadas, o atalho `/nfl` e os ajustes de degradê e chips (PR #15).
